...
async function main() {
  ...
  const existingUser = await db.prepare('SELECT id FROM "User" WHERE name = ?').get('demo') as { id: number } | undefined
  ...
  if (existingUser) {
    ...
    const result = await db.prepare('INSERT INTO "User" (name, password) VALUES (?, ?)').run('demo', hashedPassword)
  ...
  for (const p of projects) {
    const existing = await db.prepare('SELECT id FROM "Project" WHERE name_project = ?').get(p.name) as { id: number } | undefined
    if (existing) {
    ...
    ).run(p.name, p.description, userId)
    const projectId = Number(result.lastInsertRowid)

    const resourceIds: number[] = []
    for (const r of p.resources) {
      const res = await db.prepare(
        'INSERT INTO "Resource" (project_id, origine_resource, price_resource) VALUES (?, ?, ?)'
      ).run(projectId, r.origine, r.price)
      resourceIds.push(Number(res.lastInsertRowid))
    }

    for (const s of p.spends) {
      const spendRes = await db.prepare(
        'INSERT INTO "Spend" (project_id, name_spend) VALUES (?, ?)'
      ).run(projectId, s.name)
      const spendId = Number(spendRes.lastInsertRowid)

      for (const d of s.details) {
        const detailRes = await db.prepare(
          'INSERT INTO "Detail" (spend_id, name_detail) VALUES (?, ?)'
        ).run(spendId, d.name)
        const detailId = Number(detailRes.lastInsertRowid)

        for (let i = 0; i < d.makes.length; i++) {
          const price = d.makes[i]
          if (price > 0 && resourceIds[i]) {
            await db.prepare(
              'INSERT INTO "Make" (detail_id, resource_id, price_spend) VALUES (?, ?, ?)'
            ).run(detailId, resourceIds[i], price)
          }
        }
      }
    }

    console.log(`Project "${p.name}" created with ${p.resources.length} resources, ${p.spends.length} spend categories`)
  }
